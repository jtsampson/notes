...
OpenJDK binaries can be distributed sa part of a container (whereas the Oracle JDK cannot be).

- Oracle JDK Binaries will be release every three years (LTS versions)

## Time Based Release-Versioning of the OpenJDK
...
Starting with Java 10, Oracle has implemented a time based release schedule for Java.
 ...
 - Version Number: $FEATURE.$INTERIM.$UPDATE.$PATCH
  - $FEATURE = Counter, Feature version, eg, 10 and 11 for `JDK 10` and `JDK 11`, etc.
...
will result in an `java.lang.UnsupportedOperationException` runtime exception.

### toUnmodifiable()

Additional methods added to `java.util.stream.Collectors` to collect a Stream into an unmodifiable `List`, `Map` or `Set`
Any attempt to modify such a Collection will result in an `java.lang.UnsupportedOperationException` runtime exception.
```text
...
Begining with Java 10 you can mark a local variables (with initializers) with `var` instead of the actual type and 
the compiler will infer the proper type for you. There is no overhead for `var` (which is a type and not a keyword),
type is set at compile time and cannot be changed later.
...
In Java 10, you can configure a container to use a subset of the host operating system’s CPUs, and the JVM will be able
to determine the number of CPUs in use. You also can explicitly specify the number of processors the containerized JVM 
is able to see, using the `-XX:ActiveProcessorCount` flag.

## Application Class Data Sharing

Application Class data sharing has been expanded to from system classes to all available classes. At first run the JVM
...
attractive to developers, and to reduce the differences between those builds and Oracle JDK builds.
